...
function Info() {
    ...
    function query(values){
        // console.log(errors);
        if(errors.number === "" && errors.cvv === "" && errors.date === ""){
        axios.post("http://localhost:3001/getInfo", {
          ...
          .then(async (response) => {
            if(response.status == 200){
                    ...
                    // <NavigationContainer>
                        navigate('/getInfo',{ replace: true, state: {card:response.data.card,operations:response.data.operations}});
                    // </NavigationContainer>
    ...
    <form action="#" className='main-content' onSubmit={submitHandle}>
        ...
        <section className="form-item">
           <button type="submit" className='papil-button' ><Icon icon="material-symbols:info-outline" className='main-icon'/>Gauti informaciją</button>
           </section>
